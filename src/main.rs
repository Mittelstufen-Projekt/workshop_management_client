...
use models::project_material::{self, ProjectMaterial as r_ProjectMaterial};
...
fn main() -> Result<(), slint::PlatformError> {
    ...
    ui.global::<Backend>().on_route_to_project_view({
        ...
        move || {
            ...
            let project_materials = match workshop_handle.lock().unwrap().get_project_materials(&token) {
            ...
            let mut slint_projects: Vec<ProjectModel> = projects.iter().map(|p| {
                // Get the client for the project                
                let client: Result<r_Client, Error> = match clients.iter().find(|c| c.id == p.client_id) {
                    Some(client) => Ok(client.clone()),
                    None => {
                        Err(Error::new("Client not found".to_owned(), 404))
                    }
                };
                ...
                let project_materials = project_materials.iter().filter(|m| m.project_id == p.id).collect::<Vec<&r_ProjectMaterial>>();

                let materials: Vec<Material> = project_materials.iter().map(|m| {
                    let material: Result<r_Material, Error> = match materials.iter().find(|mat| mat.id == m.material_id) {
                        Some(material) => Ok(material.clone()),
                        None => {
                            Err(Error::new("Material not found".to_owned(), 404))
                        }
                    };

                    let material = match material {
                        Ok(material) => {
                            let m_type: Result<r_MaterialType, Error> = match material_types.iter().find(|t| t.id == material.type_id) {
                                Some(m_type) => Ok(m_type.clone()),
                                None => {
                                    Err(Error::new("Material type not found".to_owned(), 404))
                                }
                            };
            ...
            ui.global::<Backend>().set_recentProjects(ModelRc::new(VecModel::from(slint_projects)));
            ...
            // Set the material alerts
            ui.global::<Backend>().set_materialAlerts(ModelRc::new(VecModel::from(material_alerts)));
        }
    });
    ...
    ui.global::<Backend>().on_route_to_warehouse_management({
        ...
        move || {
            ...
            // Get the materials
            let materials: Result<Vec<r_Material>, Error> = workshop_handle.lock().unwrap().get_materials(&token);
            ...
            let slint_materials: Vec<Material> = materials.iter().map(|m| {
                let m_type: Result<r_MaterialType, Error> = workshop_handle.lock().unwrap().get_material_type_by_id(m.type_id, &token);
            ...
            ui.global::<Backend>().set_materials(ModelRc::new(VecModel::from(slint_materials)));
        }
    });
    ...
    ui.global::<Backend>().on_route_to_project_i({
        ...
        move |project_id: i32| {
            ...
            let client = workshop_handle.lock().unwrap().get_client_by_id(project.client_id, &token);
            ...
            let project_materials = workshop_handle.lock().unwrap().get_project_materials(&token);
            ...
            let materials: Vec<Material> = project_materials.iter().map(|m| {
                let material: Result<r_Material, Error> = materials.iter().find(|mat| mat.id == m.material_id).cloned().ok_or(Error::new("Material not found".to_owned(), 404));
                let material = match material {
                    Ok(material) => {
                        let m_type: Result<r_MaterialType, Error> = material_types.iter().find(|t| t.id == material.type_id).cloned().ok_or(Error::new("Material type not found".to_owned(), 404));
                        let m_type = match m_type {
                            Ok(m_type) => {
                                MaterialType {
                                    id: m_type.id,
    ...
    ui.global::<Backend>().on_editMaterial({
        ...
        move |material_id: i32| {
            ...
            let material: Material = match materials.iter().find(|m: &&r_Material| m.id == material_id) {
                Some(material) => {
                    let m_type: Result<r_MaterialType, Error> = material_types.iter().find(|t| t.id == material.type_id).cloned().ok_or(Error::new("Material type not found".to_owned(), 404));
                    let m_type = match m_type {
                        Ok(m_type) => {
                    ...
                    let project_material: Option<&r_ProjectMaterial> = project_materials.iter().find(|m| m.material_id == material_id && m.project_id == project.id);
    ...
    ui.global::<Backend>().on_deleteMaterial({
        ...
        move |material_id: i32| {
            ...
            let result = workshop_handle.lock().unwrap().delete_project_material(project_material.id, &token);
    ...
    ui.global::<Backend>().on_saveClient({
        ...
        move |first_name: SharedString, last_name: SharedString, phone: SharedString| {
            ...
            let result = workshop_handle.lock().unwrap().create_client(client, &token);
